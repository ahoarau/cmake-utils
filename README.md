...
`jrl-cmakemodules-v2` is a collection of CMake functions and macros designed to drastically reduce the boilerplate required to set up modern, production-ready C++ libraries.
...
* **Compatiblity**: Be compatible with Ubuntu 22.04 LTS which ships `CMake 3.22`. Users should be able to use `jrl-cmakemodules-v2` without upgrading CMake, and only use `apt install cmake` to get started.
...
* **Simplify Packaging**: Exporting a C++ library so it can be used via `find_package()` is notoriously difficult. `jrl-cmakemodules-v2` automates the generation of config files, version files, and transitive dependency management.
...
jrl_target_set_default_compile_options(mylib PRIVATE)
jrl_target_enforce_msvc_conformance(mylib PUBLIC)
jrl_target_generate_config_header(mylib PUBLIC)
jrl_target_headers(mylib PUBLIC HEADERS include/mylib/mylib.hpp BASE_DIRS include)
jrl_add_export_component(NAME Core TARGETS mylib)
```
...
You can manually install `jrl-cmakemodules-v2` by cloning the repository and running the following commands:

```shell
git clone https://github.com/ahoarau/jrl-cmakemodules-v2.git
cmake -S jrl-cmakemodules-v2 -B jrl-cmakemodules-v2-build
cmake --build jrl-cmakemodules-v2-build --target install
cmake --install jrl-cmakemodules-v2-build --prefix /where/to/install/jrl-cmakemodules-v2-install
```
...
```shell
cd myproject
cmake -B build -DCMAKE_PREFIX_PATH=/where/to/install/jrl-cmakemodules-v2-install
```

Then, in your `CMakeLists.txt`, you can find the package as usual:

```cmake
find_package(jrl-cmakemodules-v2 CONFIG REQUIRED)
...
You can include `jrl-cmakemodules-v2` directly in your project using CMake's `FetchContent` module. This is convenient because it requires no external installation steps.

```cmake
include(FetchContent)
FetchContent_Declare(
    jrl-cmakemodules-v2
    GIT_REPOSITORY https://github.com/ahoarau/jrl-cmakemodules-v2.git
    GIT_TAG        main
    EXCLUDE_FROM_ALL
)
FetchContent_MakeAvailable(jrl-cmakemodules-v2) # This includes the main module
```

### Method 2: `find_package`

If `jrl-cmakemodules-v2` is already installed on your system (e.g., via a package manager like Pixi/Conda, or manually via `cmake --install`), you can use `find_package`.
...
# │ Options
# └──────────────────────────────────────────────────────────────────────────────
jrl_option(MYPROJECT_BUILD_TESTS "Build tests" ON)

# ┌──────────────────────────────────────────────────────────────────────────────
# │ Dependencies
# └──────────────────────────────────────────────────────────────────────────────
jrl_find_package(fmt CONFIG REQUIRED)
...
# └──────────────────────────────────────────────────────────────────────────────
jrl_add_export_component(NAME mylib TARGETS mylib)
jrl_export_package()
...
Plain CMake               | jrl-cmakemodules-v2 | extra functionnality |
-- | -- | --
option()                  | jrl_option() | Summary table, backwards compatibility |
cmake_dependent_option()  | jrl_cmake_dependent_option() | Summary table, backwards compatibility |
find_package()            | jrl_find_package() | Automatic dependency export, summary table|
Move exe to `build/bin/`, libs to `build/lib/` | jrl_configure_default_build_dirs() | |
include(GNUInstallDirs) | jrl_configure_default_install_dirs() | |
FILE_SET                 | jrl_target_headers() | Same interface, needed as FILE_SET is CMake 3.23, and we aim 3.22 |
find_package(Python 3.8 COMPONENTS Interpreter Development.Module) | jrl_find_python(3.8 COMPONENTS Interpreter Development.Module) | Diagnostic output |
Nanobind boilerplate | jrl_find_nanobind() |  |
...
#### `jrl_configure_default_install_prefix(<path>)`
...
#### `jrl_option(<name> <description> <default> [COMPATIBILITY_OPTION <old_name>])`
...
Prints a formatted table of all options defined via `jrl_option`, showing their current values, types, and descriptions.

#### `jrl_cmake_dependent_option(<name> <description> <default> <condition> <else_value>)`
...
#### `jrl_find_package(<package> ...)`

Wraps the standard `find_package()` to allow later introspection. It records everything, from the call signature, to the imported targets, to the imported variables. This information is then used by `jrl_export_package` to automatically handle transitive dependencies in the generated CMake config files.

#### `jrl_print_dependencies_summary()`

Prints a list of all packages found via `jrl_find_package`, including the imported targets and their properties. Useful for debugging include paths and link libraries.

---

### Target Configuration

#### `jrl_target_set_default_compile_options(<target> <visibility>)`
...
#### `jrl_target_generate_config_header(<target> <visibility>)`
...
#### `jrl_target_generate_warning_header(<target> <visibility>)`
...
#### `jrl_target_generate_deprecated_header(<target> <visibility>)`

Generates a `deprecated.hpp` with macros to mark functions or classes as deprecated in a cross-platform way.

#### `jrl_target_generate_tracy_header(<target> <visibility>)`
...
#### `jrl_target_headers(<target> <visibility> HEADERS <list> BASE_DIRS <list>)`
...
#### `jrl_add_export_component(NAME <name> TARGETS <targets>)`
...
* Generates dependency files that automatically `find_dependency` any dependencies recorded by `jrl_find_package`.
...
#### `jrl_python_generate_init_py(<target> OUTPUT_PATH <path>)`
...
#### `jrl_python_compile_all(DIRECTORY <dir> [VERBOSE])`
...
#### `jrl_python_compute_install_dir(<output_var>)`
...
#### `jrl_check_python_module(<module_name> [REQUIRED] [QUIET])`
...
Manually writing `find_dependency()` calls is tedious and error-prone. `jrl-cmakemodules-v2` automates this with a two-phase approach that tracks dependencies at configure time and generates the correct `find_dependency()` calls at install time.

---

### Phase 1: Recording Dependencies (`jrl_find_package`)

When you call `jrl_find_package(fmt CONFIG REQUIRED)`, the function does the following:
...
4. **Stores Metadata as JSON**: It saves this information into a global CMake property (`_jrl_<PROJECT_NAME>_package_dependencies`) as a JSON structure.
...
### Phase 2: Exporting Dependencies (`jrl_export_package`)

When you call `jrl_export_package()`, the function performs dependency analysis at *install time* using a generated CMake script:
...
jrl_find_package(fmt CONFIG REQUIRED)    # Records: fmt -> fmt::fmt
jrl_find_package(spdlog CONFIG REQUIRED) # Records: spdlog -> spdlog::spdlog
